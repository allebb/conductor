...
* ``--stream`` - With ``conductor new``, ``destroy``, ``dump``, ``load``, ``enable``, or ``disable``, target a stream configuration in ``/etc/conductor/streams`` instead of an application virtual host.
* ``--format=json`` - With ``conductor versions`` or ``conductor stats``, return machine-readable JSON instead of text output.
* ``--streams`` - With ``conductor list``, list enabled and disabled Nginx stream configurations instead of applications.
...
```shell
conductor new myapp --fqdn="mywebapp.com www.mywebapp.com" --environment="production" --mysql-pass="Password1234" --git-uri="https://github.com/bobsta63/pastie.git" --auto-reload
```
