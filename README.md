...
Database credentials created by Conductor are stored separately as root-only files under ``/etc/conductor/credentials/{app name}.json``. The directory is mode ``0700`` and each record is mode ``0600``. Backup archives can therefore contain private keys and database passwords and are also written with mode ``0600``; treat the archives as secrets when copying them off the server. During a database restore, Conductor drops and recreates the application database and user from the archived record before importing the SQL dump. Application database dumps exclude server-global GTID state. Restore also removes legacy ``GTID_PURGED`` statements, so backups created by older Conductor releases can be imported without conflicting with transactions already executed on the destination server.
